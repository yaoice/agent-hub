...
> 聚合腾讯云智能体平台（LKE / 私有化 ADP）等 Provider 的运营指标，以 **项目（Project）** 为中心，提供空间盘点、应用盘点、Token 消耗与对话记录的可视化数据面板。
...
Agent Hub 是一套面向智能体运营场景的 **Web 看板系统**。它将不同 Provider（如腾讯云 LKE / 私有化 ADP）的运营数据统一归一化，按项目维度落库为历史快照，并以可视化面板对外呈现。
...
| 对话记录 | `DescribeMsgLogList` | 对话明细列表 + 按天趋势 + 意图分布统计 |
...
flowchart TB
    ...
    Cloud["腾讯云 LKE / 私有化 ADP<br/>(ListSpace/ListApp/Token/MsgLog)"]
...
以「项目按范围手动同步」为例，展示从前端触发到落库、回退 Mock 的完整链路：

```mermaid
sequenceDiagram
    participant U as 管理员（前端）
    participant R as Router /projects/{id}/sync
    participant G as 限频/权限校验
    participant S as Service 层
    participant P as Provider（LKE）
    participant M as Mock 数据
    participant DB as 数据库

    U->>R: POST /sync { scopes }
    R->>G: require_project_admin + 对话限频检查
    G-->>R: 通过（否则 403/429）
    R->>S: sync_dashboard / sync_conversations
    S->>P: 解密凭证后实时拉取
    alt 调用成功
    ...
    S->>DB: 写入快照 / 去重入库对话记录
    S-->>R: 汇总 { source, details }
    R-->>U: SyncResult（含 live/mock 标记）
```

> 对话记录还支持 **异步任务模式**（`conversation-sync-jobs`）：后台执行、写回进度，前端轮询展示同步进度条。
...
- **对话记录**：分页查询对话明细（支持按应用 / 时间 / 关键词 / 意图过滤），并提供按天趋势与意图分布图表。
- **管理员面板**：
  - **用户管理**：列表/搜索、详情（含所属项目）、新增、编辑、角色分配、禁用/启用。
  - **项目管理**：项目 CRUD（选 Provider + 录凭证）、成员管理、**手动同步**（按范围勾选：应用数量 / 应用对话记录 / Token 消耗，支持全选，按勾选动态生成请求）。
...
| 同步 | `POST /api/projects/{id}/sync` | 按范围统一同步 | project_admin |
| 对话同步 | `POST /api/projects/{id}/sync-conversations` | 同步对话记录 | project_admin |
| 对话异步任务 | `POST/GET /api/projects/{id}/conversation-sync-jobs[/latest|/{job_id}]` | 后台同步 + 进度轮询 | project_admin / 成员 |
| 对话查询 | `GET /api/projects/{id}/conversations`、`.../conversation-apps`、`.../conversation-stats` | 列表 / 应用下拉 / 统计 | 成员 |
...
POST /api/projects/{id}/sync
{
  ...
  "full": false                                       // 对话同步：true=全量回补，false=增量
}
```

- `app_count`（应用数量）与 `token`（Token 消耗）为看板数据，按 scope **局部拉取** 并与最新快照合并，未勾选部分沿用历史，保证看板始终完整。
- `conversations`（应用对话记录）遍历应用拉取并以 `record_id` 在项目维度 **去重入库**。
- 返回 `{ ok, source, message, scopes, details }`，`details` 含各部分同步明细。

### 增量与限频

- 对话同步默认 **增量模式**：以已入库的最大 `msg_create_time` 作为本次起点，仅拉取新增量。
...
- **SSRF 防护**：项目 HOST 默认禁止解析到内网/私有地址（含 `9./10./11./21./30.` 等网段），确需内网网关时设 `ALLOW_INTERNAL_HOSTS=true`。
